...
extern "C" {
    // 上游原为 `static _NSConcreteStackBlock: Class;`（Class 为空枚举）。
    // 「未指定（uninhabited）类型的静态变量」是 future-incompat 项
    // （rust-lang/rust#74840），未来将成为硬错误。
    // 这里只取该符号的**地址**（isa = &_NSConcreteStackBlock，从不读它的值），
    // 因此把类型改成裸指针后行为与原版完全一致，同时消除告警。
    static _NSConcreteStackBlock: *const Class;
...
impl<A, R, F> ConcreteBlock<A, R, F> {
    ...
    unsafe fn with_invoke(invoke: unsafe extern "C" fn(*mut Self, ...) -> R,
        ...
        ConcreteBlock {
            base: BlockBase {
                // 取运行时全局 _NSConcreteStackBlock 的地址（isa 指向它本身，
                // 不读其值）；符号地址与原版一致。
