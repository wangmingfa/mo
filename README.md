...
除了基础浏览，Mo 还提供一组面向「真实复杂场景」的特色能力——它们不是文件管理器的标配，而是为了解决具体痛点而生：

#### 📥 暂存区（收集夹）

从八个文件夹里各挑三个文件、再一次性操作，是文件管理器最别扭的时刻。`⌘⇧S` 把当前选择**累加**进一个进程级清单（列出每个条目的来源目录，自动去重）。清单常驻侧栏抽屉，随时可：

* 整批**复制**到当前目录（保留清单，继续往别处放）
* 整批**移动**到当前目录（清空清单）
* 单独移除某条 / 全部清空

所有动作复用既有的传输管线，所以跨端点（本地↔远程）的判定、冲突处理、进度都不用重写。

#### 🗺️ 磁盘地图（Treemap）

「谁吃了我的磁盘」用图回答。`⌘,` → 磁盘用量里切换 **方块图** 模式：用 Bruls 等人的 squarified 算法把目录树铺成面积正比于大小的色块，目录是灰蓝、各类文件按类型着色。点色块下钻、双击打开；色块比例与容器大小解耦，缩放窗口不会变形。建树带节点上限、不跟随软链、按大小排序，保证大目录也不卡。
...
`⌘⇧F` 对**当前目录子树**做真正的按内容搜索，不依赖索引：二进制嗅探跳过、正则 / 大小写 / 整词 / 上下文行可切换，命中处高亮、显示命中行片段，回车直接跳进该文件并选中。只在回车时跑（不「输入即搜」），避免每敲一个字扫一遍整棵子树把机器占死；一轮在跑时再按回车会先取消旧的再起新的。
...
Mo 只有一个「废纸篓」。无论从哪里删除——⌘⌫、右键「移到废纸篓」、去重清理、
文件夹同步的多余文件——都走同一条路，不需要选择「进哪个废纸篓」。

**它能做什么：**

* **可撤销**：删除后按 ⌘Z 撤销，文件立刻回到原位。
* **可见**：侧栏「回收站」面板列出所有被删的条目（原名、原位置、删除时间）。
* **可还原**：面板里选一条还原；原位置已不存在时会自动重建父目录。
* **可永久删除 / 清空**：面板里单条永久删除，或一键清空——两者都会先弹确认卡，
  误按 Delete / E 不会直接把文件抹掉。
* **可预览 / 打开**：面板里按空格快速预览（Quick Look），双击用系统默认应用
  打开（目录则在当前窗口浏览）——不必先还原才能查看内容。
...
* 在 Finder 的废纸篓里能看到 Mo 删的文件，Finder 的「清倒废纸篓」也照常
  生效（对应条目的磁盘空间被释放）；
* 反过来，**Finder 删的文件不会出现在 Mo 的回收站面板里**——Mo 只列自己
  账本上的条目，不猜测其他应用删除时的原始位置；
* 如果账本上的文件已被 Finder 清倒，还原时会提示文件已不在，不会报错卡死。

账本本身是 `~/.mo-trash/index.json`，只存索引不存文件——被删文件始终躺在
系统废纸篓里，Mo 不私藏副本。
...
* [x] 主题系统（浅色 / 深色 / 跟随系统 + `config.json` 自定义主题；⌘⇧P「主题…」实时预览）
* [x] 自定义布局（侧边栏 / 状态栏 / 斑马纹 / 默认视图开关 + 列序列宽持久化，「布局…」设置器）
* [x] 自定义快捷键（28 个动作可重映射 / 解绑，「快捷键…」设置器捕获按键 + 冲突检测）
